...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // Initialize the elements (Element selector)
    ...
    // Initialize the shuffle mode
    let isShuffleMode = true;
  
    // Initialize the repeat mode
    ...
    // Update play/pause button to show pause icon when playing, and play icon when paused.
    ...
    // Update the highlight on the currently playing song in the list
    ...
    function loadSong(index) {
      ...
      // Use onloadedmetadata to update duration once metadata is loaded (overwriting previous handler)
    ...
    // Navigate to previous track, wrapping to the end if necessary
    function prevSong() {
    ...
    // Navigate to the next track based on the playback mode.
    // isBtnClicked is true when the user clicks the next button.
    function nextSong(isBtnClicked) {
      if (isShuffleMode) {
        ...
        if (isRepeatMode) {
          // Repeat the same track by resetting its time.
          audio.currentTime = 0;
          progress.value = 0;
          audio.play();
          return;
        } else {
          // Play next track naturally with boundary wrap-around.
    ...
    // Helper to pad the time with zeros
    ...
    function updateProgress() {
      ...
      const remainingTime = duration - currentTimeValue;
      progress.value = currentTimeValue;
      currTime.textContent = formatTime(currentTimeValue);
    ...
    // Render the list of songs dynamically.
    function renderSongList() {
      ...
      songs.forEach((song, index) => {
        ...
        // Add classes and attributes for styling
        ...
        trackTitleElem.classList.add("track-title");
        ...
        // Construct the list item structure
        itemImg.appendChild(imgElement);
        itemImg.appendChild(thumbnailImg);
        trackDataContainer.appendChild(trackTitleElem);
    ...
    // Open/close the custom playback speed dropdown
    function toggleDropdown() {
      const dropdownList = document.getElementById("dropdown-list-items");
    ...
    // Open the preview modal and populate its content.
    ...
    // Event listeners for player controls and other interactions.
